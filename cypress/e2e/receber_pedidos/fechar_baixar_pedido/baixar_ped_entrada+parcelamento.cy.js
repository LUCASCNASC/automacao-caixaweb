...
describe('Baixar pedido: sem frete, com entrada + parcelamento', () => {
    ...
    context('Baixar pedido: sem frete, com entrada + parcelamento', () => {

        it('Baixar pedido: sem frete, com entrada + parcelamento', () => {
    ...
    context('Baixar pedido: sem frete, com entrada + parcelamento', () => {
        ...
        it.only('Baixar pedido: sem frete, com entrada + parcelamento', () => {
            ...
            cy.origin('http://10.7.0.87/#', () => {
                ...
                //clicar no botão "Avançar"
                cy.get('#frmRecebimentoPedidos_BtnBaixaPedidoVenda_material_button > .mat-button-wrapper').click({force:true}).wait(10000)

                //clicar para selecionar Baixar o pedido
                cy.get('.dx-data-row > [aria-describedby="dx-col-92"]').click({force:true}).wait(500)

                //clicar para selecionar Baixar o pedido
                cy.get('#mat-checkbox-4 > .mat-checkbox-layout > .mat-checkbox-inner-container').click({force:true})

                //clicar no botão Avançar, após selecionar para baixar
                cy.get('#frmRecebimentoPedidos_BotaoAvancar_material_button > .mat-button-wrapper').click({force:true})

                //Resumo - clicar no botão Botão
                cy.get('#frmRecebimentoPedidos_BotaoWizardConcluir_material_button > .mat-button-wrapper').click({force:true}).wait(1000)

                //Clicar em SIM, para confirmar baixa do pedido
                cy.get('.swal2-confirm').click({force:true}).wait(56000)
